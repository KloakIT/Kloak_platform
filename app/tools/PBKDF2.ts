...
(() => {
	...
	const util = {
		...
		rotl: ( n: number, b: number ) => {
			return ( n << b ) | ( n >>> ( 32 - b ))
		},
	
		// Bit-wise rotate right
		rotr: ( n: number, b: number ) => {
			return (n << ( 32 - b )) | ( n >>> b )
		},
		...
		endian: ( n: number| any ) => {
			...
			if ( n.constructor == Number ) {
				return util.rotl( n as number, 8 ) & 0x00FF00FF |
					   util.rotl( n as number, 24 ) & 0xFF00FF00
			}
	...
	const HMAC = ( hasher: any, message: any, key: any, options: any ) => {
	...
	const SHA1 = ( message: string, options: { asBytes: string , asString: string } ) => {
		...
		for ( let i = 0; i < m.length; i += 16 ) {
			...
			for ( let j = 0; j < 80; j++ ) {
				...
				if ( j < 16 ) {
					...
					let n: any = w[j-3] ^ w[j-8] ^ w[j-14] ^ w[j-16]
					w [ j ] = ( n << 1 ) | ( n >>> 31 )
				}
	...
	Crypto.PBKDF2 = ( password: any, salt: any, keylen: number, options: { hasher: any, iterations: any, asBytes: boolean, asString: boolean  } ) => {
		...
		const PRF = ( password: number[], salt: number[] ) => {
			return HMAC ( hasher, salt, password, { asBytes: true })
		}
		// Generate key
		let derivedKeyBytes: any[] = []
		let blockindex = 1
