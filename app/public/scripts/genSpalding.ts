class genSpalding {
	private currentPage = ko.observable('home')
	private totalImages = 25
	private carouselImageBase = 'carousel-image-'
	private carouselImages = ko.observableArray([])
	private currentCarouselIndex = ko.observable(0)
	private lastCarouselIndex = ko.observable(0)
	private mobileMenuOpen = ko.observable(false)
	private contactEmail = ko.observable('')
	private contactName = ko.observable('')
	private contactEmailSelected = ko.observable(false)
	private contactNameSelected = ko.observable(false)
	private contactMessage = ko.observable('')
	private tweetTimeline = ko.observableArray([
		{
			created_at: 'Tue Jul 07 15:33:25 +0000 2020',
			id: 1280525336384544800,
			id_str: '1280525336384544770',
			full_text:
				'What about those hurt by unemployment? Or business owners who lose everything? Why would you destroy people’s lives for your own political prerogatives? Main reason why no one should vote for politicians who’ve been in DC longer than 10 years.  https://t.co/rODtry8I3W',
			truncated: false,
			...
			user: {
				...
				description:
					'Author of STEALTH WAR, Fmr Senior Director for Strategy at NSC, USAF B-2 Stealth Pilot https://t.co/GXSsucnn9r',
				...
				profile_image_url:
					'http://pbs.twimg.com/profile_images/1133572528230850560/rCvjdGW1_normal.jpg',
				profile_image_url_https:
					'https://pbs.twimg.com/profile_images/1133572528230850560/rCvjdGW1_normal.jpg',
				profile_banner_url:
					'https://pbs.twimg.com/profile_banners/14308180/1590092325',
		...
		},
		{
			...
			full_text:
				'Interesting Op-Ed. My take: 1. Obviously did not read the National Security Strategy as the first 3 recommendations are in there; 2. "Talk to China" recommendation shows the naivety of a "seasoned" diplomat captured by Beijing\'s narrative.\n\nhttps://t.co/A8TDPijTy4',
			truncated: false,
				...
				urls: [
					{
						...
						expanded_url:
							'https://warontherocks.com/2020/07/what-should-come-after-trumps-failed-china-policy/?utm_source=Sailthru&utm_medium=email&utm_campaign=EBB%2007.07.20&utm_term=Editorial%20-%20Early%20Bird%20Brief',
	...
	private allBlogs = ko.observableArray([
		{
			blog_id: 1,
...

		{
			...
			title:
				'How the CCP controls: Technology, Fear, and the Pursuit of Wealth',
			short_desc:
				'Why would Michael Bloomberg say that Xi is not a dictator? Why do some Americans believe China is a democracy? It is difficult for people to fathom how a party of 80 million can control a population of almost 1.4 billion...',
			long_text: [
...

		{
			...
			short_desc:
				'Every journey we have is a series of single steps.  If we are lucky, we find fellow travelers to share the path.',
			long_text: [
	...
	private filteredBlogs = ko.observable(this.allBlogs())
	private selectedBlog = ko.observable(null)
	private categoryFilter = ko.observable('')
	...
	getDate = (date: string) => {
		try {
			...
			const date_created: Date = new Date(date)
			const time_difference: number =
				date_now.getTime() - date_created.getTime()
	...
	selectCarouselImage = (index: number, init: boolean = false) => {
		...
		const carouselMenuImage = document.getElementById(
			`${this.carouselImages()[index]}-menu`
		)
		...
		if (!init) {
			if (this.lastCarouselIndex() !== this.currentCarouselIndex()) {
				const lastCarouselMenuImage = document.getElementById(
					`${this.carouselImages()[this.lastCarouselIndex()]}-menu`
				)
